...
%filter_empty_strings(L) ->
%  filter_empty_strings(L, []).

%filter_empty_strings([], Acc)    -> lists:reverse(Acc);
%filter_empty_strings([[]|T], Acc -> filter_empty_strings(T, Acc);
%filter_empty_strings([H|T], Acc) -> filter_empty_strings(T, [H | Acc]).

build_list(H, T0, Index) ->
  T = [lists:nth(Index, L) || L <- T0],
  flatten([H | T]).
...
escape(<<$\\>>) -> <<"\\\\">>;
escape(Other) ->
  io:format("Escaping ~p~n", [Other]),
  escape(Other, <<>>).

escape(<<>>, Acc) -> Acc;
escape(<<B:1/binary, Rest/binary>>, Acc) ->
  escape(Rest, << (escape(B))/binary, Acc/binary >>).
...
Grammar
  = __ initializer:(Initializer __)? rules:(Rule __)+ {
    [_, {_, Initializer0}, {_, Rules}] = Node,
    Initializer = case Initializer0 of
                    []     -> <<>>;
                    [I, _] -> I
                  end,
    #grammar{ initializer = Initializer
            , rules       = flatten(build_list([], Rules, 1))
            , index       = Idx
            }
  }
  /*{
  ...
  = first:ActionExpression rest:(__ "/" __ ActionExpression)*
    {
      ...
      case Rest of
        []      -> First;
        Actions ->
          #choice{ alternatives = build_list(First, Actions, 4)
                 , index        = Idx
                 }
      end
    }
...
ActionExpression
  = expression:SequenceExpression code:(__ CodeBlock)?
    {
      [{_, Expression}, {_, Code0}] = Node,
      Code = case Code0 of
               []     -> <<>>;
               [_, C] -> C
             end,
      #action{ expression = flatten(Expression)
             , code       = Code
             }
    }
  ...
  = first:LabeledExpression rest:(__ LabeledExpression)*
    {
      ...
      case Rest of
        []          -> First;
        Expressions ->
          #sequence{ elements = build_list(First, Expressions, 2)
                   , index    = Idx
                   }
      end
    }
  ...
  = operator:PrefixedOperator __ expression:SuffixedExpression
    {
      ...
      #prefixed{ type       = ops_to_prefixed_types(Operator)
               , expression = Expression
               , index      = Idx
               }
    }
  ...
  = expression:PrimaryExpression __ operator:SuffixedOperator
    {
      ...
      #suffixed{ type       = ops_to_suffixed_types(Operator)
               , expression = Expression
               , index      = Idx
               }
    }
  ...
  = operator:SemanticPredicateOperator __ code:CodeBlock
    {
      ...
      #prefixed{ type  = ops_to_semantic_predicate_types(Operator)
               , code  = Code
               , index = Idx
               }
    }
...
MultiLineComment
  = "/*" (!"*/" SourceCharacter)* "*/" { iolist_to_binary(Node) }
...
IdentifierName "identifier"
  = first:IdentifierStart rest:IdentifierPart*
    {
      [{_, First}, {_, Rest}] = Node,
      lists:foldl( fun(B, Acc) -> <<Acc/binary, B/binary>> end
                 , <<>>
                 , flatten([First | Rest]))
    }
...
DoubleStringCharacter
  = !('"' / "\\" / LineTerminator) SourceCharacter { iolist_to_binary(Node) }
  / "\\" sequence:EscapeSequence { [_, {_, Sequence}] = Node, escape(iolist_to_binary(Sequence))}
  / LineContinuation
...
CharacterClassMatcher "character class"
  = "["
    ...
    ignoreCase:"i"?
    {
      ...
      Parts = iolist_to_binary(Parts0),
      #regexp{ parts       = Parts
             , raw_text    = <<"[", Parts/binary, "]">>
             , inverted    = Inverted == <<"^">>
             , ignore_case = IgnoreCase == <<"i">>
             , index       = Idx
             }
    }
/*
  ...
  = begin:ClassCharacter "-" end:ClassCharacter
    {
      ...
      case Begin > End of
        ...
        false ->
          <<Begin/binary, "-", End/binary>>
          %#character_range{ 'begin'  = Begin
          %                , 'end'    = End
          %                , raw_text = <<"[", Begin/binary, "-", End/binary, "]">>
          %                , index    = Idx
          %                }
      end
    }
...
HexEscapeSequence
  = "x" digits:$(HexDigit HexDigit)
    {
      [_, {_, Digits}] = Node,
      {ok, Num, []} = io_lib:fread("~16u", binary_to_list(Digits)),
      unicode:characters_to_binary(Num)
    }
